    ...
    public class UAClientHelperAPI
    {
        ...
        private static List<string[]> ParseDataToTagsFromDictionary(List<Object> varList, byte[] byteResult)
        {
            ...
            foreach (object val in varList)
            {
                ...
                {
                    //long[] tempArray = new long[arraylength-2];
                    string[] tempArray = new string[arraylength-2];
                    int[] timeArray = new int[arraylength - 2];

                    for (int i = 0; i < arraylength-2; i++)
                    {
                        //tempArray[i] = byteResult[index];
                        tempArray[i] = Convert.ToString(byteResult[index], 16);
                        timeArray[i] = Convert.ToInt16(tempArray[i]);
                        index += 1;
                    }
                    index += 2;
                    DateTime dt = new DateTime(timeArray[0]+2000, timeArray[1], timeArray[2], timeArray[3], timeArray[4], timeArray[5]);
                    dataReferenceStringArray[1] = String.Format("{0:yyyy-dd-MM hh:mm:ss}", dt);
                    //dataReferenceStringArray[1] = String.Concat(dataReferenceStringArray[1], ":");
                    arraylength = 0;
                }
                else if (((string[])val)[1] == "String" && arraylength > 0)
